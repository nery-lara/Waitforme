    ...
    when Answered
      @response = Twilio::TwiML::VoiceResponse.new do |response|
        ...
        response.dial(hangupOnStar: @call.hangupOnStar) do |dial_back|
          dial_back.conference('conference', muted:@con_call.muted, beep:@con_call.beep, statusCallbackEvent:@con_call.statusCallbackEvent, statusCallback:@con_call.statusCallback, statusCallbackMethod:@con_call.statusCallbackMethod)
        end
        response.redirect(@call.wait_for_business)
      end
